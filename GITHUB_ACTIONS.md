...
- **自动签到**: 每天北京时间 21:15、21:45 自动执行签到（GitHub Actions 可能有几分钟到十几分钟延迟）
...
### 自动运行
配置完 Secrets 后，Actions 会在每天北京时间 21:15 和 21:45 自动执行。GitHub Actions 定时任务可能有几分钟到十几分钟延迟，第二次执行可以降低第一次排队延迟造成的漏打卡概率。
...
schedule:
  ...
  - cron: '15 13 * * *'  # 北京时间 21:15
  - cron: '45 13 * * *'  # 北京时间 21:45
```
